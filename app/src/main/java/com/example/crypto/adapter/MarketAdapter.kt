...
import com.example.crypto.fragment.MarketFragment
import com.example.crypto.fragment.MarketFragmentDirections
import com.example.crypto.fragment.WatchListFragment
...
class MarketAdapter(var context: Context, var list: List<CryptoCurrency>, var type: String) :
    ...
    override fun onBindViewHolder(holder: MarketViewHolder, position: Int) {
        ...
        holder.binding.currencyPriceTextView.text =
            String.format("$%.02f", item.quotes?.get(0)?.price ?: 0.0)

        item.quotes?.get(0)?.let { quote ->
            ...
            if (percentChange24h != null) {
                if (percentChange24h > 0) {
                    ...
                    holder.binding.currencyChangeTextView.text = "+ ${String.format("%.02f", percentChange24h)} %"
                } else {
